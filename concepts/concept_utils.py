...
from learn_concepts_multimodal import get_concept_data, clean_concepts
...
def get_concept_texts_conceptnet(all_classes, num_recurse, CONCEPT_TEXTS_PATH):
    if not os.path.exists(CONCEPT_TEXTS_PATH):
        with open(CONCEPT_TEXTS_PATH, 'r') as f:
            all_concepts = (f.read()).split('\n')
    else:
        ...
        for i in range(1, num_recurse):
            all_concepts = get_concept_data(all_concepts)
            all_concepts = list(set(all_concepts))
            all_concepts = clean_concepts(all_concepts)
            all_concepts = list(set(all_concepts).difference(set(all_classes)))

        np.savetxt(CONCEPT_TEXTS_PATH, all_concepts, fmt='%s')

    all_concepts = [i for i in all_concepts if i != ""]
    return all_concepts

def get_concept_texts_ram(all_classes, CONCEPT_TEXTS_PATH):
    all_concepts = []
    for c in all_classes:
        with open(CONCEPT_TEXTS_PATH+c+'.txt', 'r') as f:
            all_concepts += (f.read()).split('\n')
