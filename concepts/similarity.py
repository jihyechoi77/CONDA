# source: https://github.com/Trustworthy-ML-Lab/CLIP-dissect/blob/main/similarity.py
        ...
        for t_i in tqdm(range(math.ceil(target_feats.shape[1] / batch_size))):
            ...
            for c_i in range(math.ceil(clip_feats.shape[1] / batch_size)):
                curr_similarities.append(
                    curr_target @ clip_feats[:, c_i * batch_size:(c_i + 1) * batch_size].to(device))
...
def rank_reorder(clip_feats, target_feats, device="cuda", p=3, top_fraction=0.05, scale_p=0.5):
    ...
    with torch.no_grad():
        ...
        for orig_id in tqdm(range(target_feats.shape[1])):
            clip_indices = clip_feats.gather(0, inds[:, orig_id:orig_id + 1].expand([-1, clip_feats.shape[1]])).to(
                device)
            ...
            baseline_diff = sorted_target - torch.cat(
                [sorted_target[torch.randperm(len(sorted_target))] for _ in range(5)], dim=1)
